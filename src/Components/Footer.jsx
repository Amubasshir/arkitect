...
export default function Footer() {
  ...
  const handleMouseEnter = () => {
    ...
    gsap.to(hoverTextRef.current, {
      ...
      delay: 0.6,
      onComplete: () => {
        // ALWAYS show MORE TEMPLATES at the end
        gsap.set(defaultTextRef.current, { y: '0%', opacity: 1 });
      },
    });
  };
  ...
  const handleMouseLeave = () => {
    ...
    // Reset texts so MORE TEMPLATES is always visible
    gsap.set(defaultTextRef.current, { y: '0%', opacity: 1 });
    gsap.set(hoverTextRef.current, { y: '100%', opacity: 0 });
  };
      ...
      <div ref={footerContentRef} className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-20 lg:gap-32 text-left">
          ...
          <div className="w-full md:w-1/3">
            <p className="text-sm font-bold mb-8 uppercase tracking-wide">arkitect®</p>
          ...
          {/* Right Column - Navigation */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 w-full md:w-2/3">
            {/* PAGES */}
            <div>
